...
($\textrm{if}_i \rightarrow \textrm{then}_i, tf_i, p_i$)
...
\vspace{1mm} 
and $tf_i \in$ \{True,False\} denotes, if $\textrm{if}_i$ is true, the expected truth of $\textrm{then}_i$ (here, True),
...
Dropping the $S$ parameter for convenience:
 \begin{myquote}
  consistency(B,C) = \\
\hspace*{2mm}  $- \sum_{c_i \in C} \{~ p_i ~ | ~ c_i = (\textrm{if}_i \rightarrow \textrm{then}_i, tf_i, p_i )       ~\& \\
...
\end{myquote}
If $\textrm{if}_i$ is true, \textrm{then} $tf_i \in$ \{True,False\} denotes the expected truth of $\textrm{then}_i$, 
with penalty $p_i$ if it is not.
